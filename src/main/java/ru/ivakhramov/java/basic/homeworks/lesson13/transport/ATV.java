...
public class ATV implements Transport {
    ...
    public ATV(double gas, double consumptionCoef) {
        type = "Вездеход";
        this.gas = gas;
    ...
    // 8. У каждого вида транспорта есть местности по которым он не может перемещаться: вездеход - нет ограничений
    ...
    public boolean canMove(double energy, double distance, Terrain terrain) {
        ...
        System.out.println(type + " проехал " + distance + " км. по " + terrain.getType() + ". Текущее количество топлива " + gas + " л.");
        return true;
    }
